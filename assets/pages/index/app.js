...
function initializeDashboard(data) {
    ...
    // Auto-load first point so sidebar isn't empty on page load
    if (dataGlobal.length > 0) {
        setTimeout(() => {
            toggleSidebar(false, dataGlobal[0], true);
        }, 150);
    }
...
function toggleSidebar(show, obj = null, forceHidden = false) {
    const sidebar = document.getElementById('analysisSidebar');
    if (show === undefined) show = !sidebar.classList.contains('active');
    const icon = document.getElementById('toggleIconSide');
    if (icon) icon.className = show && !forceHidden ? 'fas fa-chevron-right' : 'fas fa-chevron-left';
    if (show && !forceHidden) {
        sidebar.classList.add('active');
    } else {
